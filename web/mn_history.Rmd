...
if(nrow(data) > 0) {
  ...
  # Load history
  aqi_hist <- readRDS("../data/aqi_history2.Rdata")
  ...
  mode_hour <- table(data$Time) %>% sort(decreasing = T) %>% names() %>% as.numeric() %>% head(1)
  ...
  if(!testing) {
    aqi_hist <- filter(aqi_hist, as.Date(Date, format = '%m/%d/%Y') >= (max_date - 1))
  }
  
  #-- Average concentrations for each hour in case of duplicate results
  aqi_hist <- aqi_hist %>%
              group_by(Date, Time, AqsID, `Site Name`, Agency, Parameter) %>%
              summarize(Concentration = mean(Concentration, na.rm = T),
                        AQI_Value     = conc2aqi(Concentration, Parameter))
  ...
  saveRDS(aqi_hist, "../data/aqi_history3.Rdata")
  ...
  for (i in 1:nrow(aqi_hist)) {
    ...
    aqi_hist[i, "obs_avg"] <- mean(obs, na.rm = T) %>% round()
  ...
  if(testing) {
    aqi_hist <- filter(aqi_hist, as.Date(Date, format = '%m/%d/%Y') == as.Date(aqi_hist$Date, format = '%m/%d/%Y')[7])
  } else {
    aqi_hist <- filter(aqi_hist, as.Date(Date, format = '%m/%d/%Y') == max_date)
  }
  
  #-- Select summary values for current hour
  aqi_hist <- group_by(aqi_hist, Date, AqsID, `Site Name`, Parameter) %>%
  ...
  wide_tbl <- left_join(wide_tbl, 
                        select(aqi_hist[!duplicated(paste(aqi_hist$AqsID, aqi_hist$Parameter)), ], Date, `Site Name`, Parameter, `Daily Max 8-hr Avg`, `Rolling 24-hr Avg`, `NowCast AQI`))
...
### __PM2.5 data__  
Concentrations in units of  _$ug/m^3$_. Hours are the monitoring start hour. <br>  
