...
# Add missing sites
if(sum(!data$AqsID %in% aqi_hist$AqsID, na.rm=T) > 0) {

  new_hist <- data[!duplicated(data$AqsID), c(3,4,6)]
  new_hist <- new_hist[!new_hist$AqsID %in% aqi_hist$AqsID, ]
  hours <- names(aqi_hist[4:ncol(aqi_hist)])
  for(i in hours){new_hist[ , i] <- NA}
  new_hist$Parameter <- "PM25"
  new_hist <- arrange(new_hist, `Site Name`)
  new_hist_ozone <- new_hist
  new_hist_ozone$Parameter <- "OZONE"
  new_hist <- rbind(new_hist, new_hist_ozone)
  aqi_hist <- rbind(aqi_hist, new_hist)
  aqi_hist <- arrange(aqi_hist, desc(Parameter), `Site Name`)
  saveRDS(aqi_hist, "../data/aqi_history.Rdata")
}

  aqi_hist$`Rolling Avg` <- NULL
  aqi_hist$`NowCast AQI` <- NULL
  
  # Clear old values
  #old_hours <- as.character(max(data$Time, na.rm = T):23)
  #old_hours <- format(strptime(old_hours, "%H"), "%H")
  
  if(as.numeric(max(data$Time, na.rm = T)) == 23 && 0 %in% as.numeric(data$Time)) data <- filter(data, Time < 10)
  
  aqi_hist <- aqi_hist[ , 1:min(ncol(aqi_hist), as.numeric(max(data$Time, na.rm = T)) + 3, na.rm = T)]
  ...
  aqi_hist$` ` <- "::"
  n_col <- ncol(aqi_hist)
  aqi_hist <- aqi_hist[ , c(1:(n_col - 3), n_col, (n_col - 2), (n_col - 1))]
...
initComplete = JS(
  ...
  kable(arrange(filter(aqi_hist, Parameter == "PM25")[ , -1], `Site Name`), 'html',  align = paste0(c('l', 'l', rep('c', n_col - 3)), collapse=""))
...
initComplete = JS(
  ...
  kable(arrange(filter(aqi_hist, Parameter == "OZONE")[ , -1], `Site Name`), 'html', class="stripe nowrap compact", align = paste0(c('l', 'l', rep('c', n_col - 3)), collapse=""))
