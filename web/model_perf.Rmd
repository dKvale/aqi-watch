...
### __PM2.5 data__  
Observations and model forecasts are shown in AQI units. MPCA forecasts shown for Tuesday through Saturday have numeric values and are Day 1 forecasts. Sunday and Monday forecasts are typically only categorical and are Day 2 for Sunday and Day 3 for Monday. <br>
```{r model_perf_pm25, message=FALSE, echo=FALSE, results='asis', warning=FALSE, fig.width=10, fig.height=30 }
library(ggplot2)
...
mp1 <- ggplot(data=aqi_models_pm25, aes(x=forecast_date)) +
       ...
       geom_rect(aes(xmin=forecast_date-0.25, xmax=forecast_date+0.25, ymin=fcst_pm25_aqi_cats_val-49, ymax=fcst_pm25_aqi_cats_val,
                     colour="2"),
                 size=1.25,fill=NA) +
...
### __Ozone data__  
Observations and model forecasts are shown in AQI units. MPCA forecasts shown for Tuesday through Saturday have numeric values and are Day 1 forecasts. Sunday and Monday forecasts are typically only categorical and are Day 2 for Sunday and Day 3 for Monday. <br>
```{r model_perf_ozone, message=FALSE, echo=FALSE, results='asis', warning=FALSE, fig.width=10, fig.height=25 }
...
mo1 <- ggplot(data=aqi_models_ozone, aes(x=forecast_date)) +
       ...
       geom_rect(aes(xmin=forecast_date-0.25, xmax=forecast_date+0.25, ymin=fcst_ozone_aqi_cats_val-49, 
                     ymax=fcst_ozone_aqi_cats_val, colour="2"),
                     size=1.25,fill=NA) +
